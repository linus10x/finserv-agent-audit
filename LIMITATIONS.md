...
9a. **No named production deployment; no examiner has reviewed a chain produced by this code in situ.** The "built for examination" claim is about the artifact's *design* — the control surface is falsifiable under audit, demonstrated by the runnable `./demo.sh` (a forged grant, a deleted revocation, an in-place mutation, and a backdated regeneration each caught) — not about a completed regulatory examination or an operational track record. The demo proves the *mechanism*; it does not substitute for a first real deployed-and-examined engagement.

9b. **The authority lifecycle checks evidence PRESENCE, not ADEQUACY.** `verify_authority_invariants` confirms a grant carries evidence and that the evidence matches its recorded digest (the recorded evidence is the evidence that was hashed). It does NOT validate that the evidence is sufficient, true, or that it clears the A2→A3 promotion criteria. "Granted only against evidence" means *recorded* evidence, not *adequate* evidence — adequacy is the deployer's second-line responsibility, evaluated by `autonomy_ladder.check_a2_to_a3_promotion`. For the A3+ regulator-visible boundary, the deployer should pass the serialized output of that gate as the grant evidence (the `./demo.sh` example does); `grant()` does not itself re-run the gate (hard coupling is an ADR-0026 follow-up).

9c. **Examiner independence is enforced only when actor_ids are recorded; otherwise it is a deployer control.** When both the grant and the examination carry an `actor_id`, `verify_authority_invariants` rejects an examination performed by the grantor. With actor_ids omitted, independence cannot be verified by this module — recording distinct grantor / examiner identities is a deployer responsibility. The EXAMINE event is the record slot for an SR 11-7 effective challenge (a `passed` flag plus a free-text finding); the module records the challenge artifact, it does not perform or grade the challenge.
