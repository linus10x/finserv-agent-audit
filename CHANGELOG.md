...
### Added
- **Demotion-gated authority lifecycle** (`governance/authority_lifecycle.py`, ADR-0026). `AuthorityLifecycle` records GRANT / EXAMINE / REVOKE as first-class hash-chained events: authority is granted only against recorded evidence, a granted level is examined (a recorded examination finding), and a revocation is recorded against the grant and the finding that triggered it (authority going *down*, recorded). New `AuditEventType` members `AUTHORITY_GRANTED`, `AUTHORITY_EXAMINED`, `AUTHORITY_REVOKED`.
- **`verify_authority_invariants`** — semantic verification above hash-chain integrity. Rejects a forged grant (no evidence, non-JSON-native evidence, or an evidence/`evidence_digest` mismatch), a revocation referencing an absent grant/examination, **and an examination whose examiner `actor_id` equals the grantor's** (enforced when both are recorded — an examination must be independent of the grant it challenges) — *even in a chain whose hashes were regenerated to pass `verify()`*.
- **External-anchor verification** (`governance/witness_anchor.py`): `verify_against_external_anchors` defeats head-truncation and backdated regeneration in one check — an honest append-only chain only grows, so a witnessed head must still be present. `RecordingWitness` + `ExternalAnchorRecord` retain anchored heads offline (stand in for Rekor / OpenTimestamps / a regulator-side log), so verification needs no network. **Fail-closed**: runs `verify()` first and raises on a hash-inconsistent chain, so calling it alone cannot pass a chain `verify()` would reject.
- **Runnable demotion-gate demo + `./demo.sh`** (`examples/demotion_gate_demo.py`). Clone-and-run, no install / no network / no credentials, self-verifying (exits non-zero if any expected catch fails): builds the lifecycle (the A3 grant's evidence is the **output of the real `check_a2_to_a3_promotion` gate**, and the examiner is a distinct second-line actor), anchors it, and proves four attacks are each caught — forged grant, deleted revocation, in-place mutation, backdated regeneration. The README 30-second tour now leads with it.
...
- **`verify()` / `verify_strict()` no longer crash on adversarial payloads** — a forged event with a non-JSON-native payload (e.g. a planted `set`) now yields a tamper verdict (`False` / `AuditChainTamperError`) instead of an uncaught `TypeError`, honoring the "returns False if tampered" contract. `evidence_digest` likewise requires JSON-native evidence (dropped `default=str`), so a digest re-computes byte-identically across hosts.
...
- Claim scoping (per a model-risk adversarial review): "granted only against evidence" means **recorded** evidence (presence + digest integrity, not adequacy/validity — adequacy is the deployer's second-line gate); the EXAMINE event is the **record slot** for an SR 11-7 effective challenge, not the challenge itself; examiner independence is enforced only when actor_ids are recorded, a deployer control otherwise. Documented in the module docstring + LIMITATIONS.
- Test suite: 661 passing (was 630), `mypy --strict` clean, 93% coverage.

## [2.3.0] — STAGED (release date + tag + Zenodo DOI set by owner at tag time — OWNER GATE)

**Autonomy Ladder primitive-hardening release (S1).** Closes three primitive defects and two overclaimed compliance claims found by the G-SIB-scale assurance-catalog pressure-test. MINOR bump from the live tag `v2.2.0` (no default/mode was flipped — every hardening is a default-preserving opt-in; the only contract-narrowing change rejects an insecure misuse, see *Changed*). Bumped `pyproject` (2.2.0 → 2.3.0), `__version__`, and `CITATION.cff`. **Note:** the P3 `AuditChain.verify()` genesis-awareness defect this release originally targeted was independently fixed by the shipped v2.2.0 release (`_expected_chain_seed`); this branch's own P3 fix (`_verify_seed_for_first`) was dropped as a duplicate during the rebase onto v2.2.0 rather than landed alongside it.

> **DOI note.** A new version-specific Zenodo DOI auto-mints for v2.3.0 on Release publish under the existing concept DOI (10.5281/zenodo.20434570). NEVER re-tag the already-DOI'd v1/v2.1/v2.2 artifacts.

### Fixed
- **P2 — `SovereignVeto` thread-safety gap in `clear()`.** `trigger()` / `is_vetoed` / `active_vetos` / `history` were already lock-guarded; `clear()`'s mutation walk was not, so a concurrent risk-monitor `trigger()` could collide with an operator `clear()` iteration (the veto's whole purpose is concurrent operation). `clear()` now holds the same `threading.RLock` for its mutation walk; the Authorizer check stays outside the lock so a slow IdP call does not serialize the gate. Pinned by `tests/test_sovereign_veto.py::test_concurrent_trigger_and_clear_no_race`. Surfaced by the S1 §8 stress chamber.

### Added
- **P3 — production mode (`AuditChain(production=True)`).** A named strict opt-in that FAILS CLOSED at construction unless the chain is both deployer-keyed AND anchored to an external `witness_register` — because end-to-end regeneration is detectable only via an out-of-band witness (ADR-0014), not the internally-consistent hash chain alone. Default (`production=False`) is byte-for-byte the v1.x advisory contract.
- **P2 — sovereign-veto production mode (`SovereignVeto(production=True)`).** Fails closed without a wired `Authorizer`, so `clear()` is gated by an authenticated principal and `operator_id` on the chain is bound to that identity. Default unchanged (advisory + warning). Veto-state persistence/recovery posture documented (replay triggers/clears from the ledger).
- **P1 — independent-attestation strict mode for the A2→A3 level-gate.** `check_a2_to_a3_promotion(..., require_attestation=True, attestations=...)` requires each satisfied criterion to carry an independent (line-2/3) `CriterionAttestation` or it fails. The default check is now explicitly labelled **advisory** (`PromotionGateReport.advisory=True`) — it trusts caller-asserted inputs and is not an enforcing control. New: `CriterionAttestation`.
- **P5 — challenger-independence attestation (`ChallengerIndependence`).** The effective-challenge harness records WHO chose the challenger, WHEN, and the not-same-owner / vendor-family / prompt-template attestation onto the audit chain; `require_independence=True` fails closed without a full attestation.
- **Adversarial AL-PROBE suite.** `tests/adversarial/test_al_probes.py` re-authors the five G-SIB-scale assurance-catalog probes as committed, reproducible tests (the originals were ephemeral `/tmp` scripts).
- **§7 property + golden-corpus tiers.** `tests/test_primitive_properties.py` (hypothesis, ~2,000 generated cases across the five primitives) + `tests/test_golden_corpus_fair_lending.py` (real, primary-source-cited DOJ redlining + CFPB ECOA adverse-action matters-of-record exercised against `equity_audit` / `adverse_action_gate`).
